...
let run_executable args executable =
  let cmd = sprintf "%s%s %s" dot_path executable args in
  sh cmd (fun _ i -> i)
...
let process_ll_ast path file ll_ast =
  ...
  then (Platform.verb "..compiling with clang\n";
    ...
    Platform.verb "..compiling with backend\n";
    let asm_ast = Backend.compile_prog ll_ast in
    let asm_str = X86.string_of_prog asm_ast in
    if !print_regs_flag then begin
      let (h,size) = Registers.histogram_of_prog asm_ast in
      Printf.printf "%s\n" (Registers.string_of_histogram h);
      Printf.printf "Summary: %d\n" (Registers.summary h);
...
let process_oat_ast path file oat_ast =
  ...
  Platform.verb @@ Printf.sprintf "..typechecking";
  Typechecker.typecheck_program oat_ast;
  Platform.verb @@ Printf.sprintf "..frontend";
