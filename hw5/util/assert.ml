...
let timeout_assert_const (a: assertion) : assertion = 
    timeout_assert 10 a

let assert_eq v1 v2 : assertion =
  timeout_assert_const (fun () -> if v1 <> v2 then failwith "not equal" else ())

let assert_eqf f v2 : assertion =
  timeout_assert_const (fun () -> if (f ()) <> v2 then failwith "not equal" else ())

let assert_eqfs f v2 : assertion =
  timeout_assert_const (fun () ->
    let s1 = f () in
